...
<?php
  ...
  require_once dirname(dirname(__FILE__)).'/EPaymentMerchant_PHP/UCB_UniPlatba/UniPlatbaPaymentHttpResponse.class.php';
  $pres = new UniPlatbaPaymentHttpResponse();
  
  if ($pres->Validate()) {
    echo 'Received message is valid.<br />';
    
    if ($pres->VerifySignature(UCB_UNIPLATBA_SHAREDSECRET)) {
