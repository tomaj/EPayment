...
*/
	...
	$returnUrl = $protocol.'://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['PHP_SELF']).'/return_vub_eplatba.php';
...
<html>
<head>
	<title>VUB Eplatba / MONOGRAM EPayment Libraries for PHP</title>
...
<body>
	...
	<div class="container">
		<div class="page-header">
			<h1>VUB Eplatba <small> MONOGRAM EPayment Libraries for PHP</small></h1>
		</div>
...
<?php
	foreach ($vubFields as $key => $value) {
...
<?php
	if (isset($_POST['dopayment'])) {
		require_once dirname(dirname(__FILE__)).'/EPaymentMerchant_PHP/VUB_EPlatba/EPlatbaPaymentRequest.class.php';
		...
		$pr->SetRedirectUrlBase(VUB_EPLATBA_REDIRECTURLBASE);
		...
		if ($validationResult) {
			...
			$pr->SignMessage(VUB_EPLATBA_SHAREDSECRET);
