...
<?php
	...
	require_once dirname(dirname(__FILE__)).'/EPaymentMerchant_PHP/VUB_EPlatba/EPlatbaPaymentHttpResponse.class.php';
	$pres = new EPlatbaPaymentHttpResponse();
  
	if ($pres->Validate()) {
		echo '<div class="alert alert-success">Received message is valid.</div>';
    
		if ($pres->VerifySignature(VUB_EPLATBA_SHAREDSECRET)) {
