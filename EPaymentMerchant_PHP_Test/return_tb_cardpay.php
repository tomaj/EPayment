...
<?php
  ...
  require_once dirname(dirname(__FILE__)).'/EPaymentMerchant_PHP/TB_CardPay/CardPayPaymentHttpResponse.class.php';
  $pres = new CardPayPaymentHttpResponse();
  
  if ($pres->Validate()) {
    echo 'Received message is valid.<br />';
    
    if ($pres->VerifySignature(TB_CARDPAY_SHAREDSECRET)) {
